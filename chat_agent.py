...
class ChatAgentMagics(Magics):
    ...
    def __init__(self):
        ...
        tools = load_tools(
            ["google-serper"],
            llm=self.__llm
        ) if 'SERPER_API_KEY' in os.environ else []
        self.__tools = tools + [
            Tool.from_function(
    ...
    N - this code does not produce the right answer
                ...
                while not feedback_retrieved:
                    try:
                        ...
                        if first_input == 'n':
                            response_end = 'most likely because it doesn\'t achieve the desired result.' if len(user_feedback) == 0 else f' and has the following feedback: {user_feedback}'
            ...
            if 'result' in environment:
                ...
                if self.__verbose:
                    print('The variable `result` was not found in executing environment. Using the assignment on the last code line instead for the result.')
            ...
            response_end = '' if len(user_feedback) == 0 else f' - The user has the following feedback: {user_feedback}'
            self.__last_key = result_key
            return f'Answer has been successfully derived. Key: {result_key}{response_end}' if not type(result) == str else result + response_end
    ...
    def chat_agent(self, line: Optional[str], cell: Optional[str]=None):
        ...
        parser.add_argument(
            '--noninteractive', '-n', action='store_true',
...
''' if len(self.__agent_input) > 0 else '') + available_variables
        cell = cell.strip()
        print('Prompt:')
        print(cell)
